...
- **Record/replay executor wrapper** (`AUTODEV_RECORD_DIR` / `AUTODEV_REPLAY_DIR` env vars): record real Codex/Claude responses to a dir during normal runs; replay them deterministically later. Wraps all 7 executors via `RecordingExecutorWrapper` with atomic-temp-file writes for crash safety. New `src/autodev/executors/recording_executor.py`.
- **Self-MCP-call** — `SelfMcpClient` spawns `autodev-x mcp-serve` as a subprocess and exposes a Python API for an agent to invoke autodev-x's own MCP tools recursively; `SelfMcpAgent` wraps the client with roundtable-compatible output. Falls back to deterministic mock when `FACTORY_FORCE_MOCK=1` or binary absent. New `src/autodev/adapters/self_mcp_client.py` + `src/autodev/agents/self_mcp_agent.py`.
- `autodev-x deliver-multi --config <yaml>` subcommand: run `deliver-project` semantics across N repos defined in a YAML manifest (`group_name` + per-repo `path`/`brief_file`/`languages`); writes per-repo artifacts + aggregate `summary.json` under `.dev-factory/multi-runs/<group_id>/`. New `src/autodev/flows/multi_repo_flow.py`.
- `autodev-x reverse-doc --watch <src>` subcommand: single-pass or watched continuous brownfield doc generation. Wraps existing `DocumentProjectAgent` with the project's `_watcher.py` debouncer; default debounce 1.0 s. Output to configurable `--out` dir (default `docs/_auto`). New `src/autodev/flows/reverse_doc_watcher_flow.py`.
- **Tailnet A2A transport** — `TailnetTransport` discovers peers via `tailscale status --json` and binds A2A endpoints to the host's tailnet IP from `tailscale ip -4`; delegates HTTP send via the existing `A2AHttpTransport` with `allow_private_networks=True`. Raises `TailnetUnavailable` with install hint when `tailscale` binary is absent. New `src/autodev/adapters/a2a/transports/tailnet.py`.
