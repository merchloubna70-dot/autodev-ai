"""Gemini CLI adapter — opt-in executor for the `gemini` CLI (Google).

Usage:
    autodev-x deliver-project --executor gemini ...

The `gemini` binary must be installed and available on PATH. Install it via:
    npm install -g @google/generative-ai-cli
or follow the official Gemini CLI installation instructions.
"""
...
class GeminiExecutor(BaseExecutor):
    """Opt-in executor that delegates to the `gemini` CLI (Google Gemini).

    This executor is NOT selected by ``--executor auto``; it is only used when
    the caller explicitly passes ``--executor gemini``.
    """
    ...
    def is_available(self) -> bool:
        """Return True if the ``gemini`` binary is found on PATH."""
    ...
    def execute(self, request: ExecutionRequest) -> ExecutionResult:
        if not self.is_available():
            return self._fail(
                ...
                message=(
                    f"gemini CLI not found on PATH (looked for '{self.binary}'). "
                    "Install it with: npm install -g @google/generative-ai-cli "
                    "or follow the official Gemini CLI installation instructions."
                ),
            )
        ...
        # Build argv: `gemini --prompt <prompt>` (plain text, non-interactive)
        # Gemini CLI supports `--prompt` for non-interactive mode.
        argv = [self.binary, "--prompt", prompt]
        cmd_str = f"{self.binary} --prompt <prompt_sha={short_hash(prompt, 16)}>"
        ...
        return ExecutionResult(
            ...
            safety_flags=[],
            mode=request.mode,
            selected_backend_reason=f"gemini CLI invoked (prompt_sha={short_hash(prompt, 16)})",
        )
