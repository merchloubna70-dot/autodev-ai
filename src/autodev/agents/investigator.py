...
class InvestigatorAgent:
    ...
    def collect_evidence(self, case: CaseFile) -> CaseFile:
        ...
        if kind == InvestigationInputKind.TICKET_ID:
            ...
            entries.extend(self._collect_error_msg(token, case))

        elif kind == InvestigationInputKind.CODE_AREA:
            entries.extend(self._collect_code_area(token, case))

        elif kind == InvestigationInputKind.PROBLEM_DESC:
            entries.extend(self._collect_prose(token, case))
    ...
    def run(self, input_token: str, repo_path: str) -> CaseFile:
        ...
        case = self.collect_evidence(case)
    ...
    def _collect_error_msg(self, error_msg: str, case: CaseFile) -> list[EvidenceEntry]:
    ...
    def _collect_code_area(self, area: str, case: CaseFile) -> list[EvidenceEntry]:
        """List files matching the area glob + sample git blame on first hit."""
        entries: list[EvidenceEntry] = []
        try:
            import glob as _glob
            hits = list(_glob.glob(area, recursive=True))[:10]
            for fp in hits:
                entries.append(EvidenceEntry(
                    kind="file",
                    path=fp,
    ...
    def _collect_prose(self, prose: str, case: CaseFile) -> list[EvidenceEntry]:
